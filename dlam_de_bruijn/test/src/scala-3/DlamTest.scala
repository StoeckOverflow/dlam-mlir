...
class DlamDeBruijnTests extends AnyFlatSpec {
  ...
  "Monomorphize pass" should "replace tapply with a specialized vlam and remove the use" in {
    ...
    val module: ModuleOp =
      new ModuleOp(
        operands = Seq.empty, // Seq[Value[Attribute]]
        successors = Seq.empty, // Seq[Block]
        results = Seq.empty, // Seq[Result[Attribute]]
        regions = Seq(Region(Seq(Block(operations = Seq(F))))), // Seq[Region]
        properties = Map.empty[String, Attribute],
        attributes = LinkedHashMap.empty[String, Attribute]
      )
    ...
    def count[T](op: Operation)(using m: reflect.ClassTag[T]): Int = {
      ...
      def walkOp(o: Operation): Unit = {
        if (m.runtimeClass.isInstance(o)) n += 1
    ...
    if (tRetIdx < 0) fail("no dlam.treturn found in outer block")

    // val tRetOp = outerBlock.operations
    //  .collectFirst { case tr: TReturn => tr }
    //  .getOrElse(fail("no TReturn found in outer block"))
    // tRetOp.expected shouldEqual alphaFun

    val idxSpec = outerBlock.getIndexOf(spec)
    // val idxRet = outerBlock.getIndexOf(tRetOp)
  ...
  "NatExpr printing" should "print literals and composite forms" in {
    ...
    two.custom_print shouldBe "!dlam.nat_lit<2>"
    add.custom_print shouldBe "!dlam.nat.add<!dlam.nat_lit<2>, !dlam.nat_lit<3>>"
    mul.custom_print shouldBe "!dlam.nat.mul<!dlam.nat.add<!dlam.nat_lit<2>, !dlam.nat_lit<3>>, !dlam.nat_lit<2>>"
  ...
  "NatExpr embedding" should "work as a parameter inside other attributes" in {
    ...
    t.custom_print should include("!dlam.nat.mul")
  }
  ...
  "A DlamVecType" should "print and embed nat exprs correctly" in {
    ...
    vecTy.custom_print should include("!dlam.nat_lit<4>")
    vecTy.custom_print should include("!dlam.bvar<0>")
  }
